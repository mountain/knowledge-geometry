...
$Kxy$ 解读为遗忘掉时空里的历史，只取到最后的结果，其实就是蕴含或者因果关系，如果 $K$ 单置，可以解读为时间。

\begin{principle}
恒者恒在
\end{principle}

$$I = \iota \iota = \iota S K$$

$\iota S K$ 是一个有 10 个叶节点的二叉树。

\Tree [.$\ominus$ [.$\odot$ [.$\ominus$ [.$\odot$ [.$\odot$ $\iota$ $\iota$ ] $\iota$ ] $\iota$ ] [.$\oplus$ $\iota$ $\iota$ ] ] 6 ]
