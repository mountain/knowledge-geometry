...
针对不同的具体问题， $ L $ 的给出略有不同，我们关心的情况包括：最优控制、资料同化和参数学习。

\subsubsection{最优控制}

受演化方程 $ f $ 控制的一个过程，我们需要调整边界上的控制变量 $ \mathbf{u} $ ，使得在终止时刻的计算值 $ \mathbf{y_1} $ 同目标值 $ \mathbf{y}_1^o $ 之间的误差极小。
...
受演化方程 $ f $ 控制的一个过程，我们需要调整初始时刻的 $ \mathbf{y}_0 $ ，使得在终止时刻的计算值 $ \mathbf{y_1} $ 同观测值 $ \mathbf{y}_1^o $ 之间的误差极小。
...
$$
\begin{alignat}{2}
J(y) & = \int\limits_{-1}^{+1}(y_1 - y_1^o)^2dx + \lambda \int\limits_{0}^{1}(1 + 1)dt \\
& = \int\limits_{-1}^{+1} (y_1 - y_1^o)^2dx + 2 \lambda \\
\end{alignat}
$$
