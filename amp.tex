...
\centering
\begin{tikzpicture}
    \draw (0, 0) node[inner sep=0] {\includegraphics[width=4in]{images/surreal.png}};
\end{tikzpicture}
...
1、初等数学：一个程序的计算过程体现为一个流动，用函数式的观点，重新考察初等几何、线性代数、微积分和微分方程等。比如圆是什么？

2、计算理论：Chaitin 常数 $\Omega$ 的几何位置在哪里？

3、学习理论：学习过程体现为一个反复迭代的程序，是测地线上的流动，重新考察学习的概念。

4、庞加莱圆盘可以看成复平面上的单位圆盘，黎曼共形映射定理，直觉上，应该会有一个非常简洁的证明。
