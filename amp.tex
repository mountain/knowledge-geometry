...
我们将会看到在双曲空间里，有一种美妙的结果，围绕着 H-树 的某种一笔画展开，将会填充整个空间，而且相邻各段是彼此垂直的。
这种双曲空间里填充方式和一种称为超现实数的特殊构造联系在一起。
...
如上图，我们可以引入一条乘线 $0$，该零线和原来的加线相交于中点 $0$。
...
上图中的黑线是我们恰当连接无穷远点得到的，我们声称上图中的黑线是等值线。随着 $-1$ 和 $+1$ 的出现，全体整数都出现在我们的开始寻找零点的那条加线上。
这样我们就完成了嵌入 $n$ 的任务。

\newpage

\subsection{重新看到加线和所有的倍加线}

\newpage

\subsection{重新看到乘线和所有的倍乘线}
...
Ads/CFT 对应常见的一个例子是庞加莱圆盘和边界。其实边界不应该看成一条线，正确的几何观点是看成无数条彼此垂直的测地线在边界捏合起来。
这些测地线构成希尔伯特空间，也即量子的几何结构。它是一种多世界的观点。
