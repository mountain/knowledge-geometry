...
把边界 $\partial D$ 定义为无穷远，同时把垂直于 $\partial D$ 的圆弧（弧的两端都垂直于 $\partial D$）定义为新的几何上的直线（测地线）。
...
\end{tikzpicture}
\caption{过一点 P 与直线 L 平行的和非交的直线}
\end{figure}

在此基础上，我们可以引入新的几何上的直线间的关系。
...
如上图，过直线 $L$ 外一点 $P$ 与 $L$ 平行的直线有两条，与 $L$ 非交的直线有无数条。
这个事实是双曲几何不同于欧氏几何的地方。
