...
\subsubsection{Conclusions from the generating}\label{sec:generating}
...
It is easy to see the algebraic structure $\mathcal{P} = (A_n, \circ)$ is a group, and we call it \emph{path group}.
...
\subsubsection{Conclusions from the calculation}\label{sec:calculation}

In this section, we study the conclusions related with the calculation result from each expression of $A_n$.
For example, we can verify path group $\mathcal{P}$ is non-abelian by calculating the result at point $0$.
