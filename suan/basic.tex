...
\subsection{The beginning question}\label{sec:question}

It is a fundamental goal to describe and understand changing processes in mathematics.
Newton and Leibniz introduced calculus as a tool to achieve this goal in the 1680s.

The basic concept is derivatives of functions, which linearize changes over a short period, and the accumulation of derivatives over time gives the original functions.
Back to the arithmetic level, we can treat this technique as a method through accumulating additions.

Except for the above, people also found the corresponding concept of accumulating multiplications.
Napier discovered these concepts even before the invention of calculus;
in 1614, Napier introduced logarithm function, and in 1618 he also discovered the nature exponent constant $e$.
In the mathematical descriptions of changing processes, people widely used the logarithm and exponential functions.

So let us demonstrate these accumulating method at the arithmetic level

$$ 16 = 1 + 2 + 3 + 4 + 3 + 2 + 1 $$

reflects a changing sequence of $[1, 3, 6, 10, 13, 15, 16]$, or

$$ 16 = 1 \times 2 \times 2 \times 2 \times 2 $$

reflects a changing sequence of $[1, 2, 4, 8, 16]$.

We use the same additive or multiplicative operators step by step to apply them on different operands in both cases.
We can call them homogeneous accumulation processes.

Alternatively, we have other cases like:

$$ 16 = ((1 + 2) \times 2 + 2) \times 2 = (1 \times 2 + 4) \times 2 + 4 = ...$$

In these cases, we combine both additive and multiplicative operators on a sequence of operands simultaneously,
and we call these heterogeneous accumulation processes.

We can connect the integral of derivatives to the homogeneous accumulation process,
so can we expand the calculus theory to those heterogeneous accumulation process?
That set the beginning question of our paper.

\subsection{Hyperbolic plane}\label{sec:disk}

The hyperbolic plane $H^2$ is a two-dimensional manifold with constant negative curvature.
The hyperbolic plane study was begun in the 19th century when Nikolai Ivanovich Lobachevsky,
János Bolyai, Carl Friedrich Gauss first revealed it.
Below are several geometric facts about the hyperbolic plane which we are interested in.

\subsubsection{parallel lines}

\subsubsection{cycles, hypercycles and horocycles}

\subsubsection{apeirogon}
...
\subsubsection{Poincaré upper half plane model}

\section{Order-4 apeirogonal tiling}\label{sec:tiling}

\section{Arithmetical expressions}\label{sec:expressions}

We consider the arithmetical expression set $A(a, b)$ which is freely generated from
\begin{itemize}
    \item initial operand: $0$
    \item operator: $\oplus_a: x \mapsto x + a$
    \item operator: $\ominus_a: x \mapsto x - a$
    \item operator: $\otimes_b: x \mapsto x \cdot b$
    \item operator: $\oslash_b: x \mapsto x / b$
\end{itemize}
where $a$ and $b$ is two positive real number, $a$ is the additional generator and $b$ is the multiplicative generator.

With different equality we adopt on A, we will get different structures:
\begin{itemize}
    \item literal structure: under the literal equality which leads to paths on a tree structure
    \item syntactical structure: under the syntactical equality which embrace a path group
    \item semantical structure: under the equality of real when we compare the two side after evaluate both expressions
\end{itemize}
...
After introduce of a zero operator $\odot$ which is its own inverse:
\begin{itemize}
