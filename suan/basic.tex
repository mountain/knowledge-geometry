...
\section{A beginning question}\label{sec:question}

It is a fundamental task to describe an evolving process in mathematic. After the introduce of concepts of variable and function,
Newton and Leibniz set up the area of calculus. It use a linearized small pieces to reflect the changing rate and accumulate
the small pieces of change into a larger part, that is derivitive and integral.

Back to basic arithmetical facts, as showed in some trivial examples, these kind of accumulative evolving process is happening like:
...
we call them homogeneous accumulation process.

We use same trivial examples, but from theoretical level, how should we treat arithmetical expressions like:

$$ 8 = (1 \times 2 + 1) \times 2 + 1 = (1 + 2) \times 2 + 2 = \ldots $$

In these cases, we combine both additive and multiplicative operators on sequence of operands, or heterogeneous accumulation process.

Can we expand calculus theory to those heterogeneous accumulation process? That set the beginning question of our paper.
...
\section{An assignment from canonical forms into hyperbolic surface}\label{sec:assignment}
