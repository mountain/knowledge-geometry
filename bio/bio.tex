...
特征值模平方不变是一个圆，不妨称为特征圆。这进一步引导我们去考虑如下的构造：赋值 $A$ 有等值线和梯度线，
我们交换等值线和梯度线的地位得到 $A$ 的共轭 $B$。我们对 $A$、$B$ 都施加同样的共形变换并加以观察。因为每个赋值都有零线，
$A$ 的零线变换到无穷远，我们会得到一个调和函数 $f$，类似， 把 $B$ 的零线变换到无穷远，我们会得到一个调和函数 $g$。
当我们同时观察 $A$、$B$，有种很强的直观， 就是 A 变成 f 的时候，B 正好远离 g；而 B 变成 g 的时候，正好 A 远离 f。
我们需需要通过某种方式把 $f$ 和 $g$ 组合起来，得到一个复函数，这个复函数是 Laplace 算子的一个特征向量。
有种很强的直观，$f$ 和 $g$ 出现在复函数的特征圆上体现为周期上的相位差。
...
是刻画空间；另一个和梯度-等值线坐标系联系，是刻画时间；空间可以沿着时间的线可以折叠起来，刚好就是沿着坐标系轴线来折叠。

上面提到的这两个基本域的位置差，就是前面说的共形不变的特征值圆上的相位差。但我目前还在想办法去澄清和验证这些直观的想法。
