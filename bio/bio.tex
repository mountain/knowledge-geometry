...
需证明图中存在一个二等分角的序列。理论上可以通过数学归纳法，在初等几何下证明该命题，但过于繁复，还没有走通。
...
设一个赋值是 $f$，将梯度和等值线地位互易得到共轭赋值是 $g$，然后我们研究复化以后的函数 $f+ ig$。
...
是刻画空间；另一个和梯度-等值线坐标系联系，是刻画时间；空间可以沿着时间的线可以折叠起来，刚好就是沿着两个坐标系的夹角来折叠。
