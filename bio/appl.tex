...
\begin{itemize}
    ...
    \item 03：学习计划：结合关键问题与思路，尽快补齐知识上的短板
    \item 04：研究计划：主要问题、解决思路
    \item 05：附加推荐信：维基媒体基金会前主席陈霆先生的推荐信
    \item 06：当前研究进展：初步的研究进展
    \item 07：几何式分析的工作思路：进一步工作的思路
...
那个时候认识还不深。我在尚未完成的一个文稿里有段文字写前后的经历：
...
它围成的面积和这个扰动生成的赋值有正比关系，即式\ref{eq:gradevo} 。
...
如图\ref{fig:merged1}，当我们把这种对称性和四阶无限边形铺嵌结合在一起观察的时候，
...
\end{scope}
\end{tikzpicture}
\caption{第二类例子：四阶无限边形铺嵌里的第一种对称性}\label{fig:merged1}
\end{figure}
...
\begin{scope}[line width=0.3pt]
    ...
    \tkzDrawCircle[color=black](O,V1)
    \tkzDrawArc[R with nodes,color=yellow](K1,\rAp)(C,D)
    \tkzDrawArc[R with nodes,color=yellow](K2,0.5)(V3,V4)
    ...
    \tkzLabelLine[blue,above](A,N){$s$}
    \tkzLabelPoint[above](C){$M_{e}$}
    \tkzLabelPoint[above](D){$M_{w}$}
...
\end{scope}
\end{tikzpicture}
\caption{第三类例子：四阶无限边形铺嵌里的第二种对称性}\label{fig:merged2}
\end{figure}
...
如果上面四阶无限边形铺嵌的构造问题可以顺利解决，会使得第二类例子的构造成立，这种情况下，我们可以讨论一个管型构造。\footnote{感谢刘宇老师的讨论}
