...
\begin{itemize}
    \item 例子构造：需要完成四阶无限边形的确切构造、严格给出第二、第三个例子、探讨例子和四阶无限边形染色的关系，在此基础上能够明确无疑的给出管型构造
...
\end{scope}
\end{tikzpicture}
\caption{四阶无限边形铺嵌里的第一种对称性}\label{fig:merged1}
\end{figure}
...
\end{scope}
\end{tikzpicture}
\caption{四阶无限边形铺嵌里的第二种对称性}\label{fig:merged2}
\end{figure}
...
这种必然性和科学哲学领域的 Benacerraf-Field 挑战有关系。Benacerraf-Field 挑战首先由 P. Benacerraf 提出，并由 Field 重新给与表述。
