...
很容易理解，在乘法分配律和一些项的合并与拆散后，上述几个表达式可以互相转化。
我们可以规定黑色和紫色的路径是一对对偶的典范路径，那么所有连接如上图的赋值 $1$ 和 $3$ 之间的路径，
...
因为式\ref{eq:gradevo} 可以给到 $a$ 和 $s$ 的关系，启发我们下面用火烧法去研究与零线等距的超圆族（hypercycle family）。
...
从上节的解集里，我们发现它也是满足流方程的一个解。怎么看待例子里的这个现象？

\subsection{从调和共轭开始}\label{subsec:conjugates}

\subsection{单值化下的几何观点}\label{subsec:uniformation}

\subsection{群论下的观点}\label{subsec:group}

群的结构

\subsection{更一般的讨论}\label{subsec:general}
