...
在本节，我们讨论四个例子，这些例子都可以理解成算术表达式的某种几何表达。
\begin{itemize}
    \item 在几何上，这些例子表现为一个流形
    \item 在代数上，流形的点都对应算术表达式的某种典范形态
    \item 从表达力的角度讲，任何算术表达式都对应这个流形上的一条路径或者通路
\end{itemize}

需要声明本节的陈述是不严密的，流形的点对应算术表达式的典范形态的这个表达仍然需要进一步的严密化。
对第一节实数轴来说，这样的严密化手段早已经被戴德金等人发展出来了，我们需要把以往的工作改造成表达式的情形。

\subsection{实数轴的例子}

给定实数轴 $\mathcal{R}$ 和上面的坐标$\phi: \mathcal{R} \mapsto (-\infty, +\infty)$。
