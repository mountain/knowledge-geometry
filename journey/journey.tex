...
\begin{itemize}
    \item 字面结构 $\mathcal{S}^0$：根据表达式字面上是否相等 $\mathcal{I}^0$
    ...
    \item 语义结构 $\mathcal{S}^\infty$：根据表达式运算结果的值否相等$\mathcal{I}^\infty$
\end{itemize}

引入态射后，这些不同的结构之间可以构成范畴，而字面结构 $\mathcal{S}^0$ 是初始对象，语义结构 $\mathcal{S}^\infty$ 是终止对象。
在值相等的观点下 $\mathcal{S}^\infty$ 也是一个环。同时也可以考虑这一系列结构上的扩张或者细化，直到最后完成完备化。

\subsection{通路记法}\label{subsec:walknotion}

首先我们明确图论术语通路、轨迹、路径（英文中的 walk, trail, path）的区别。
...
\caption{$\mathcal{S}^i$的生成图}
\end{figure}

生成图是四阶凯莱树，典范形式是其上的路径。
