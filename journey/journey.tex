...
\subsection{流方程}\label{subsec:flow}
...
当 $\theta$ 相对于 $s$ 取常数时，式 \eqref{eq:flow_1e} 对应的微分方程是可解的，我们在附录里给出推导的过程，这里仅仅给出解本身。
...
方程 \eqref{eq:gradient} 可解，得出 $a$ 和 $s$ 的关系如下：

\begin{equation}
    s = \frac{1}{\lambda} \ln \frac{\mu}{a + \sqrt{\mu^2 + \lambda^2 a^2}}
\end{equation}
