...
\begin{itemize}
    \item 几何上，例子是一个流形
    \item 算术上，流形上的点都对应算术表达式的某种典范形态
    \item 任何可线索化的算术表达式，都对应到流形上的一条路径
\end{itemize}
...
蓝线之间是等间隔为长度 1 的。网格里的红色数值是赋值的大小。还容易看到，这个赋值有半圆形的梯度场，而它的等值线是从原点出发的射线。
...
\end{tikzpicture}
\caption{加乘网格上的一条路径（浓黑色），$\lambda=\ln2$ 和 $\mu=1$}\label{fig:gridex0}
\end{figure}
...
首先，我们说明可以线索化的算术表达式是指其表达式树的左枝深度为一的表达式，它可以组织成一个线索，按照顺序来估值。

例如，下式因为项 $(2 + 1)$ 的存在，使得表达式树有一个左枝深度为二，所以它不是可线索化的算术表达式。
$$
(1 * 2 * 2 - 1) * (2 + 1) - 6
$$

利用分配律和项的合并，稍微变形一下，就可以得到和上式等值的可线索化的算术表达式。

$$
(1 * 2 * 2 - 1) * 2 + 3 - 6
$$

注意到蓝线代表赋值上的 $+ 1$ 关系，绿线代表赋值上的 $\times 2$ 关系，我们就能得到如下图的路径
...
对这四个例子，因为第二个例子形式比较简单，我们会着重解释它的赋值、表达式如何体现为几何上的路径以及典范形式。
...
\begin{scope}[line width=0.3pt]
    ...
    \tkzLabelLine[blue,left](O,A){$y$}
    \tkzLabelLine[blue,above](A,N){$s$}

\end{scope}
\end{tikzpicture}
\caption{从零线开始火烧}\label{fig:merged}
\end{figure}
