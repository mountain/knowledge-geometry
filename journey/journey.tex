...
如果要求相同基本关系的连线彼此必须平行，我们能得到什么？ 此时，我们发现：
...
$$
  \mathcal{R}_0 \prec \mathcal{R}_i \prec \mathcal{R}_d \prec \mathcal{R}_\infty
$$
$$
...
\begin{itemize}
    \item 字面结构 $\mathcal{S}^0$：根据表达式字面上是否相等 $\mathcal{R}_0$
    \item 语法结构：根据表达式在特定公理下是否相等
        \subitem 引入了加减互逆、乘除互逆会导致等值关系 $\mathcal{R}_i$，由此带来结构 $\mathcal{S}^i$
        \subitem 在 $\mathcal{R}_i$ 基础上再引入分配律带来等值关系 $\mathcal{R}_d$，由此得到结构 $\mathcal{S}^d$
    \item 语义结构 $\mathcal{S}^\infty$：根据表达式运算结果的值否相等$\mathcal{R}_\infty$
\end{itemize}

引入态射后，这些不同的结构之间可以构成范畴，而字面结构是初始对象，语义结构是终止对象。
同时也可以考虑某种扩张，对应到表达式，就有路径群的不断细化，直到最后完成完备化。

\subsection{路径记法}\label{subsec:pathnotion}

假设我们有一串操作符 $a_1, a_2, \cdots a_{n-1}, a_n$，我们可以引入如下的 \emph{路径记法}.

\begin{definition}
\label{definition:path}
    一个路径 $a_1 a_2 \cdots a_{n-1} a_n$ 是 $\mathcal{F}$ 上的一个函数，它依次把操作符作用到操作数上
    $$a_1 a_2 \cdots a_{n-1} a_n (x) \coloneqq a_n( a_{n-1}( \cdots a_2( a_1(x) ) \cdots ) ), \forall x \in \mathcal{F}$$
\end{definition}

于是 $E(\mu, \lambda)$ 中的表达式都可以写成作用在操作数 $0$ 的路径，并且我们可以验证一个路径内的操作符满足结合律。
引入路径的拼合 $ \cdot $ 后，路径可以构成一个半群。

我们可以把一个长的计算过程拆成小段，然后再组合小段回到原来的表达式，举例来说
...
这里 $\alpha$, $\beta$ 和 $\gamma$ 全部都定义在 $E$ 里面。 我们可以论证 $\mathcal{P} = (E, \circ)$ 是一个群，我们称之为\emph{路径群}.

\subsection{字面结构 $\mathcal{S}^0$}\label{subsec:literial}

当我们采纳了字面上的等值关系，我们得到了一个出度为 5 的生成树。\footnote[2]{这个生成树对应一个六阶无限边形铺嵌，是否有对应右手系下的符号排布，是个有趣的问题。}
...
显然，在 $\mathcal{F}$ 上加减是互逆的，乘除也是互逆的，由此我们引入操作符的逆
\begin{itemize}
    \item $\oplus^{-1} = \ominus$
    \item $\ominus^{-1} = \oplus$
...
\subsection{加细与完备化}\label{subsec:completeness}
