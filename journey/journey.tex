...
因为生成结构是树，任意两顶点间的典范路径就是连接两顶点的唯一路径。
...
从每个 E_i 中取点 x_i 使得 A(x_i) 在双曲圆盘 H 上构成柯西序列（ https://en.wikipedia.org/wiki/Cauchy_sequence ），
在此条件下，我们必须证明 x_i 在代数上的收敛性。如果度量上的收敛性蕴涵代数上的收敛性，我们称赋值 A 是完备的。

怎么来推进呢？需要借助双曲圆盘上的希尔伯特曲线。欧式空间上的希尔伯特曲线是一个逐层细密的迭代结构。
给定任意一个 H 上的柯西序列，可以在希尔伯特曲线族的逐层网格上，选取一个点列出来和原本的柯西序列收敛于同一个点。
而希尔伯特曲线族上每一个点，在赋值 A 下都可以有的代数表达式上的定义。由此就可以讨论代数上的收敛性。
