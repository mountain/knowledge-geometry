...
网格在未来的论证中还会发挥更多的作用。在分析学的视角中，它的作用如同黎曼可积条件里借助离散化求和来实现连续可积。
...
\begin{figure}[ht]
    ...
    \begin{tikzpicture}
        \begin{scope}[line width=0.3pt]
            \draw (0, 0) node[inner sep=0] {\includegraphics[width=3.142in]{images/apeirogon.png}};
        \end{scope}
    \end{tikzpicture}
...
\begin{figure}[ht]
    ...
    \begin{tikzpicture}
        \begin{scope}[line width=0.3pt]
            \draw (0, 0) node[inner sep=0] {\includegraphics[width=3.142in]{images/tube.png}};
        \end{scope}
    \end{tikzpicture}
    \caption{四阶无限边形铺嵌}\label{fig:apeirogon}
\end{figure}
...
可定义数域上的一些结构，它们和赋值结构兼容
* 输送
* 映射

与赋值兼容的映射可以实现为算术表达式空间上的流
